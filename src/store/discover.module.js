...
export default {
  ...
  getters: {
    locations: (state) =>
      state.locations.map((l) => {
        l.distance = state.userLocation
          ? util.haversine(l.coords, state.userLocation.coords)
          : undefined;
        return l;
      }),
  ...
  actions: {
    fetchLocations: async ({ commit, state }, { query, sort, filters }) => {
      locationsAPI
        .getLocations(query, sort, filters, state.userLocation)
        .then((locations) => commit("setLocations", locations));
    },

    fetchObjects: async ({ commit }) =>
      locationsAPI
        .getObjects()
        .then((objects) => commit("setObjects", objects)),
    fetchTags: async ({ commit }) =>
      locationsAPI.getTags().then((tags) => commit("setTags", tags)),
  },
};
