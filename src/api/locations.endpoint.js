...
export default {
  getLocations: async (query, sort, filters, userLocation) => {
    ...
    return axios.get(base, { params }).then((res) => res.data.locations);
  },

  getTags: async () => axios.get(base + "/tags").then((res) => res.data.tags),

  getObjects: async () =>
    axios.get(base + "/objects").then((res) => res.data.objects),
};
